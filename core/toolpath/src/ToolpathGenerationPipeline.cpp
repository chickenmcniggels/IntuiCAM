...
ToolpathGenerationPipeline::executePipeline(const PipelineInputs &inputs) {
  ...
  try {
    ...
    if (inputs.facing) {
      ...
      // Position facing at the Z-max of the raw material
      double facingStartZ = inputs.z0 + 1.0; // Start slightly ahead of stock
      double facingEndZ =
          inputs.z0 - inputs.facingAllowance; // Remove allowance

      // Determine maximum radius from profile or raw material diameter
      double facingMaxRadius = inputs.rawMaterialDiameter / 2.0;
      if (!inputs.profile2D.isEmpty()) {
        double profileMinZ, profileMaxZ, profileMinRadius, profileMaxRadius;
        inputs.profile2D.getBounds(profileMinZ, profileMaxZ, profileMinRadius,
                                   profileMaxRadius);
        facingMaxRadius = std::max(facingMaxRadius, profileMaxRadius + 2.0);
      }

      std::cout << "  - Facing coordinates: Z=" << facingStartZ << " to "
                << facingEndZ << ", radius=" << facingMaxRadius << std::endl;

      double depthOfCut = 1.0; // mm - placeholder
      int passes =
          static_cast<int>(std::floor(inputs.facingAllowance / depthOfCut));
      ...
      for (int i = 0; i < passes; ++i) {
        ...
        // IMPROVED POSITIONING: Use profile-based coordinates for accurate
        // positioning
        double axialPosition =
            facingStartZ - i * depthOfCut; // Start from profile front face

        IntuiCAM::Geometry::Point3D coordinates(axialPosition, 0.0,
                                                facingMaxRadius);
        IntuiCAM::Geometry::Point3D startPos(axialPosition, 0.0,
                                             facingMaxRadius +
                                                 2.0); // Start slightly outside
        IntuiCAM::Geometry::Point3D endPos(axialPosition, 0.0,
                                           0.0); // End at center

        auto facingPasses =
            facingToolpath(coordinates, startPos, endPos, inputs.facingTool);
      ...
      IntuiCAM::Geometry::Point3D startPos(facingEndZ, 0.0,
                                           facingMaxRadius + 2.0);
      IntuiCAM::Geometry::Point3D endPos(facingEndZ, 0.0, 0.0);
      auto finalFacing =
          facingToolpath(finalCoord, startPos, endPos, inputs.facingTool);
    ...
    if (inputs.drilling && inputs.machineInternalFeatures) {
      ...
      std::vector<double> diameters = {6.0, 8.0, 10.0,
                                       12.0}; // Example diameters
    ...
    if (inputs.internalGrooving && inputs.machineInternalFeatures) {
      ...
      for (const auto &groove : inputs.internalFeaturesToBeGrooved) {
        ...
        auto groovingPaths =
            internalGroovingToolpath(groove.coordinates, groove.geometry,
                                     groove.tool, groove.chamferEdges);
    ...
    if (inputs.externalRoughing) {
      ...
      // Start roughing at the raw material Z-max
      coordinates = IntuiCAM::Geometry::Point3D(
          inputs.z0, 0.0, inputs.rawMaterialDiameter / 2.0);

      if (!inputs.profile2D.isEmpty()) {
        double profileMinZ, profileMaxZ, profileMinRadius, profileMaxRadius;
        inputs.profile2D.getBounds(profileMinZ, profileMaxZ, profileMinRadius,
                                   profileMaxRadius);
        coordinates.z = std::max(coordinates.z, profileMaxRadius);
      }

      std::cout << "  - Roughing coordinates: Z=" << coordinates.x
                << ", radius=" << coordinates.z << std::endl;
    ...
    if (inputs.externalGrooving) {
      ...
      for (const auto &groove : inputs.externalFeaturesToBeGrooved) {
        ...
        auto groovingPaths =
            externalGroovingToolpath(groove.coordinates, groove.geometry,
                                     groove.tool, groove.chamferEdges);
        for (auto &tp : groovingPaths) {
          result.timeline.push_back(std::move(tp));
        }
    ...
    if (inputs.chamfering) {
      ...
      for (const auto &chamfer : inputs.featuresToBeChamfered) {
        ...
        auto chamferPaths = chamferingToolpath(chamfer.coordinates,
                                               chamfer.geometry, chamfer.tool);
    ...
    if (inputs.threading) {
      ...
      for (const auto &thread : inputs.featuresToBeThreaded) {
        ...
        auto threadPaths =
            threadingToolpath(thread.coordinates, thread.geometry, thread.tool);
    ...
    if (inputs.parting) {
      ...
      auto partingPaths =
          partingToolpath(partingCoordinates, inputs.partingTool, false);
    ...
    result.errorMessage =
        "Unexpected error during pipeline execution: " + std::string(e.what());
    result.success = false;
  }
...
ToolpathGenerationPipeline::extractInputsFromPart(
    const TopoDS_Shape &partGeometry, const gp_Ax1 &turningAxis) {
  PipelineInputs inputs;

  // IMPROVED: Store the actual part geometry for use by operations
  m_currentPartGeometry = partGeometry;

  // IMPROVED: Extract 2D profile from part geometry first - this is critical
  // for profile-based toolpaths
  if (!partGeometry.IsNull()) {
    ...
    std::cout << "ToolpathGenerationPipeline: Extracting 2D profile from part "
                 "geometry..."
              << std::endl;
    inputs.profile2D =
        ProfileExtractor::extractProfile(partGeometry, extractParams);

    if (!inputs.profile2D.isEmpty()) {
      std::cout << "  - Profile extracted successfully with "
                << inputs.profile2D.getSegmentCount() << " segments"
                << std::endl;
    } else {
      std::cout << "  - Warning: Profile extraction returned empty profile"
                << std::endl;
    }
  }
  ...
  if (!partGeometry.IsNull()) {
    ...
    if (!partBox.IsVoid()) {
      ...
      inputs.rawMaterialDiameter =
          maxRadius * 2.1; // 105% of part size for stock allowance
      inputs.rawMaterialLength = partLength * 1.2; // 120% of part length
      inputs.z0 = inputs.rawMaterialLength;
      inputs.partLength = partLength;

      std::cout << "ToolpathGenerationPipeline: Dynamic extraction from part "
                   "geometry:"
                << std::endl;
      std::cout << "  - Part length: " << partLength << "mm" << std::endl;
      std::cout << "  - Max radius: " << maxRadius << "mm" << std::endl;
      std::cout << "  - Raw material diameter: " << inputs.rawMaterialDiameter
                << "mm" << std::endl;
      std::cout << "  - Raw material length: " << inputs.rawMaterialLength
                << "mm" << std::endl;
    } else {
      std::cerr << "ToolpathGenerationPipeline: Warning - Part geometry "
                   "bounding box is void, using fallback values"
                << std::endl;
    ...
    std::cerr << "ToolpathGenerationPipeline: Warning - No part geometry "
  ...
  for (const auto &feature : features) {
    if (feature.type == "hole") {
      inputs.featuresToBeDrilled.push_back(feature);
               ...
               feature.diameter < inputs.rawMaterialDiameter) {
...
ToolpathGenerationPipeline::detectFeatures(
    const LatheProfile::Profile2D &profile, const TopoDS_Shape &partGeometry) {
  std::vector<DetectedFeature> features;

  // DYNAMIC FEATURE DETECTION: Analyze actual part geometry instead of
  // hardcoded features
  if (!partGeometry.IsNull()) {
    std::cout
        << "ToolpathGenerationPipeline: Analyzing part geometry for features..."
        << std::endl;
    ...
    if (!profile.segments.empty()) {
      ...
      for (size_t i = 0; i < profile.segments.size(); ++i) {
        ...
        // Simple heuristic: if segment is short and potentially represents a
        // hole feature In lathe coordinates, start.x is typically the radius,
        // start.z is the axial position
        double segmentRadius = std::abs(segment.start.x);

        if (segmentRadius > 0.0 && segmentRadius < 5.0 &&
            segment.length > 0.5) {
          DetectedFeature holeFeature;
          holeFeature.type = "hole";
          holeFeature.depth = segment.length;
          holeFeature.diameter = segmentRadius * 2.0;
          holeFeature.coordinates = IntuiCAM::Geometry::Point3D(
              segment.start.z, 0.0, segment.start.x);
          ...
          std::cout << "  - Detected hole feature: diameter = "
                    << holeFeature.diameter
    ...
    std::cout << "ToolpathGenerationPipeline: No part geometry provided for "
                 "feature detection"
              << std::endl;
  }
...
// Real implementations for toolpath generation functions using actual operation
// classes
std::vector<std::unique_ptr<Toolpath>>
ToolpathGenerationPipeline::facingToolpath(
    const IntuiCAM::Geometry::Point3D &coordinates,
    const IntuiCAM::Geometry::Point3D &startPos,
    const IntuiCAM::Geometry::Point3D &endPos, const std::string &toolData) {
  ...
  // IMPROVED: Use actual coordinates from part geometry instead of hardcoded
  // values Extract geometry bounds from the current profile if available
  double startZ =
      coordinates.x; // Lathe coordinate system: X=axial (Z in machine)
  double endZ = endPos.x;
  double maxRadius =
      coordinates.z;      // Z=radius in lathe coordinates (X in machine)
  double minRadius = 0.0; // Usually face to center

  // Set up operation parameters based on actual coordinates and realistic GUI
  // parameters
  ...
  if (toolpath) {
    ...
    auto &movements =
        const_cast<std::vector<Movement> &>(toolpath->getMovements());
...
std::vector<std::unique_ptr<Toolpath>>
ToolpathGenerationPipeline::drillingToolpath(double depth,
                                             const std::string &toolData) {

  std::vector<std::unique_ptr<Toolpath>> result;

  auto tool = std::make_shared<Tool>(Tool::Type::Turning, toolData);
  auto toolpath = std::make_unique<Toolpath>("Center Drilling", tool,
...
std::vector<std::unique_ptr<Toolpath>>
ToolpathGenerationPipeline::internalRoughingToolpath(
    const IntuiCAM::Geometry::Point3D &coordinates, const std::string &toolData,
    const LatheProfile::Profile2D &profile) {

  std::vector<std::unique_ptr<Toolpath>> result;

  auto tool = std::make_shared<Tool>(Tool::Type::Turning, toolData);
  auto toolpath = std::make_unique<Toolpath>("Internal Roughing", tool,
                                             OperationType::InternalRoughing);

  // Realistic internal roughing with multiple passes
  double startDiameter =
      std::max(2.0, coordinates.z * 0.5); // Start with small internal diameter
  double finalDiameter =
      coordinates.z * 1.8; // Target diameter (90% of coordinates radius)
  ...
  int numPasses =
      static_cast<int>(std::ceil((finalDiameter - startDiameter) / depthOfCut));
  ...
  for (int pass = 0; pass < numPasses; ++pass) {
    ...
    IntuiCAM::Geometry::Point3D entryPos(coordinates.x + clearanceZ, 0.0,
                                         currentRadius);
    ...
    IntuiCAM::Geometry::Point3D endCut(coordinates.x - lengthZ, 0.0,
                                       currentRadius);
    toolpath->addLinearMove(endCut, feedRate);

    // Retract axially
    IntuiCAM::Geometry::Point3D retractPos(coordinates.x + clearanceZ, 0.0,
                                           currentRadius);
    toolpath->addRapidMove(retractPos);
  }

  // Final rapid to safe position
  IntuiCAM::Geometry::Point3D safePos(coordinates.x + clearanceZ, 0.0,
                                      coordinates.z + 5.0);
...
std::vector<std::unique_ptr<Toolpath>>
ToolpathGenerationPipeline::externalRoughingToolpath(
    const IntuiCAM::Geometry::Point3D &coordinates, const std::string &toolData,
    const LatheProfile::Profile2D &profile) {
...
std::vector<std::unique_ptr<Toolpath>>
ToolpathGenerationPipeline::internalFinishingToolpath(
    const IntuiCAM::Geometry::Point3D &coordinates, const std::string &toolData,
    const LatheProfile::Profile2D &profile) {
  ...
  // Create FinishingOperation instance with name and tool for internal
  // finishing
  FinishingOperation finishingOp("Internal Finishing", tool);

  // Set up operation parameters for internal finishing based on coordinates and
...
std::vector<std::unique_ptr<Toolpath>>
ToolpathGenerationPipeline::externalFinishingToolpath(
    const IntuiCAM::Geometry::Point3D &coordinates, const std::string &toolData,
    const LatheProfile::Profile2D &profile) {
...
std::vector<std::unique_ptr<Toolpath>>
ToolpathGenerationPipeline::externalGroovingToolpath(
    const IntuiCAM::Geometry::Point3D &coordinates,
  ...
  auto toolpath = std::make_unique<Toolpath>("External Grooving", tool,
                                             OperationType::ExternalGrooving);

  // Extract groove parameters from geometry map or use defaults
  double grooveWidth =
      grooveGeometry.count("width") ? grooveGeometry.at("width") : 3.0; // mm
  double grooveDepth =
      grooveGeometry.count("depth") ? grooveGeometry.at("depth") : 2.0; // mm
  double toolWidth = grooveGeometry.count("tool_width")
                         ? grooveGeometry.at("tool_width")
                         : 2.5; // mm
  ...
  IntuiCAM::Geometry::Point3D startPos(grooveStartZ, 0.0,
                                       coordinates.z + clearanceDistance);
  toolpath->addRapidMove(startPos);
  ...
  for (int pass = 0; pass < numPasses; ++pass) {
    ...
    IntuiCAM::Geometry::Point3D currentStart(currentZ, 0.0,
                                             coordinates.z + clearanceDistance);
    ...
    IntuiCAM::Geometry::Point3D retractPos(currentZ, 0.0,
                                           coordinates.z + clearanceDistance);
  ...
  if (chamferEdges) {
    ...
    // Left chamfer
    IntuiCAM::Geometry::Point3D leftChamferStart(grooveStartZ, 0.0,
                                                 coordinates.z);
    IntuiCAM::Geometry::Point3D leftChamferEnd(grooveStartZ - chamferSize, 0.0,
                                               coordinates.z - chamferSize);
    toolpath->addLinearMove(leftChamferStart, feedRate);
    toolpath->addLinearMove(leftChamferEnd, feedRate);

    // Right chamfer
    IntuiCAM::Geometry::Point3D rightChamferStart(grooveEndZ, 0.0,
                                                  coordinates.z);
    IntuiCAM::Geometry::Point3D rightChamferEnd(grooveEndZ + chamferSize, 0.0,
                                                coordinates.z - chamferSize);
...
std::vector<std::unique_ptr<Toolpath>>
ToolpathGenerationPipeline::internalGroovingToolpath(
    const IntuiCAM::Geometry::Point3D &coordinates,
    const std::map<std::string, double> &grooveGeometry,
    const std::string &toolData, bool chamferEdges) {
  ...
  auto tool = std::make_shared<Tool>(Tool::Type::Grooving, toolData);
  auto toolpath = std::make_unique<Toolpath>("Internal Grooving", tool,
                                             OperationType::InternalGrooving);
  ...
  double toolWidth = grooveGeometry.count("tool_width")
                         ? grooveGeometry.at("tool_width")
                         : 2.5; // mm
  double boreDiameter = grooveGeometry.count("bore_diameter")
                            ? grooveGeometry.at("bore_diameter")
                            : coordinates.z * 1.6; // mm

  double feedRate = 35.0;         // mm/min (slower for internal grooving)
  double clearanceDistance = 1.0; // mm
  double startRadius =
      boreDiameter / 2.0 - clearanceDistance; // Start inside bore

  // Calculate groove positions
  double grooveStartZ = coordinates.x - grooveWidth / 2.0;
  double grooveEndZ = coordinates.x + grooveWidth / 2.0;
  double finalRadius =
      startRadius + grooveDepth; // Internal grooving expands outward

  // Rapid to approach position (inside the bore)
  IntuiCAM::Geometry::Point3D approachPos(grooveStartZ, 0.0,
                                          0.0); // Approach from center
  toolpath->addRapidMove(approachPos);
...
std::vector<std::unique_ptr<Toolpath>>
ToolpathGenerationPipeline::chamferingToolpath(
    const IntuiCAM::Geometry::Point3D &coordinates,
  ...
  auto tool = std::make_shared<Tool>(Tool::Type::Turning, toolData);
  auto toolpath =
      std::make_unique<Toolpath>("Chamfering", tool, OperationType::Chamfering);

  // Extract chamfer parameters from geometry map or use defaults
  double chamferSize =
      chamferGeometry.count("size") ? chamferGeometry.at("size") : 1.0; // mm
  double chamferAngle = chamferGeometry.count("angle")
                            ? chamferGeometry.at("angle")
                            : 45.0; // degrees
  bool isInternal = chamferGeometry.count("internal")
                        ? (chamferGeometry.at("internal") > 0.5)
                        : false;
  bool isFrontFace = chamferGeometry.count("front_face")
                         ? (chamferGeometry.at("front_face") > 0.5)
                         : true;
  ...
  if (isInternal) {
    ...
    double boreRadius =
        coordinates.z * 0.8; // Assume 80% of coordinate radius is bore

    // Approach from center
    IntuiCAM::Geometry::Point3D approachPos(coordinates.x + clearanceDistance,
                                            0.0, 0.0);
    toolpath->addRapidMove(approachPos);

    // Move to chamfer start position
    IntuiCAM::Geometry::Point3D startPos(coordinates.x, 0.0,
                                         boreRadius - radialComponent);
    toolpath->addLinearMove(startPos, feedRate);

    // Execute chamfer move
    IntuiCAM::Geometry::Point3D endPos;
    if (isFrontFace) {
      endPos = IntuiCAM::Geometry::Point3D(coordinates.x + axialComponent, 0.0,
                                           boreRadius);
    } else {
      endPos = IntuiCAM::Geometry::Point3D(coordinates.x - axialComponent, 0.0,
                                           boreRadius);
    ...
    // Retract to center
    IntuiCAM::Geometry::Point3D retractPos(coordinates.x + clearanceDistance,
                                           0.0, 0.0);
    toolpath->addRapidMove(retractPos);
    ...
    if (isFrontFace) {
      ...
      startPos = IntuiCAM::Geometry::Point3D(coordinates.x, 0.0,
                                             coordinates.z - radialComponent);
      endPos = IntuiCAM::Geometry::Point3D(coordinates.x + axialComponent, 0.0,
                                           coordinates.z);
    } else {
      // Back face chamfer (OD to face)
      startPos = IntuiCAM::Geometry::Point3D(coordinates.x - axialComponent,
                                             0.0, coordinates.z);
      endPos = IntuiCAM::Geometry::Point3D(coordinates.x, 0.0,
                                           coordinates.z - radialComponent);
    }
...
std::vector<std::unique_ptr<Toolpath>>
ToolpathGenerationPipeline::threadingToolpath(
    const IntuiCAM::Geometry::Point3D &coordinates,
  ...
  auto toolpath =
      std::make_unique<Toolpath>("Threading", tool, OperationType::Threading);

  // Extract thread parameters from geometry map or use defaults
  double pitch = threadGeometry.count("pitch") ? threadGeometry.at("pitch")
                                               : 1.5; // mm (M10 thread)
  double threadLength =
      threadGeometry.count("length") ? threadGeometry.at("length") : 15.0; // mm
  double threadDepth = threadGeometry.count("depth")
                           ? threadGeometry.at("depth")
                           : 0.9; // mm (60% of pitch)
  double majorDiameter = threadGeometry.count("major_diameter")
                             ? threadGeometry.at("major_diameter")
                             : coordinates.z * 2.0; // mm
  bool isInternal = threadGeometry.count("internal")
                        ? (threadGeometry.at("internal") > 0.5)
                        : false;
  int numPasses = threadGeometry.count("passes")
                      ? static_cast<int>(threadGeometry.at("passes"))
                      : 3;
  ...
  if (isInternal) {
    ...
    // Approach from center
    IntuiCAM::Geometry::Point3D approachPos(threadStartZ + clearanceDistance,
                                            0.0, 0.0);
    toolpath->addRapidMove(approachPos);
    ...
    for (int pass = 0; pass < numPasses; ++pass) {
      ...
      // Retract axially
      IntuiCAM::Geometry::Point3D retractPos(threadEndZ - clearanceDistance,
                                             0.0, currentRadius);
      toolpath->addRapidMove(retractPos);

      // Return to start position for next pass
      if (pass < numPasses - 1) {
        toolpath->addRapidMove(IntuiCAM::Geometry::Point3D(
            threadStartZ + clearanceDistance, 0.0, 0.0));
      }
    }

    // Final retract to center
    IntuiCAM::Geometry::Point3D finalRetract(threadStartZ + clearanceDistance,
                                             0.0, 0.0);
    ...
    for (int pass = 0; pass < numPasses; ++pass) {
      ...
      if (pass < numPasses - 1) {
        toolpath->addRapidMove(IntuiCAM::Geometry::Point3D(
            threadStartZ + clearanceDistance, 0.0, safeRadius));
      }
    }
...
std::vector<std::unique_ptr<Toolpath>>
ToolpathGenerationPipeline::partingToolpath(
    const IntuiCAM::Geometry::Point3D &coordinates, const std::string &toolData,
    bool chamferEdges) {
  ...
  params.partingZ =
      coordinates.x; // X position for parting in lathe coordinates
  ...
  if (partingResult.success) {
    if (partingResult.grooveToolpath) {
      ...
      auto &mv = const_cast<std::vector<Movement> &>(
          partingResult.grooveToolpath->getMovements());
      for (auto &m : mv) {
    ...
    if (partingResult.partingToolpath) {
      ...
      auto &mv = const_cast<std::vector<Movement> &>(
          partingResult.partingToolpath->getMovements());
      for (auto &m : mv) {
        m.operationType = OperationType::Parting;
        m.operationName = "Parting";
      }
    ...
    if (partingResult.finishingToolpath) {
      ...
      auto &mv = const_cast<std::vector<Movement> &>(
          partingResult.finishingToolpath->getMovements());
      for (auto &m : mv) {
        m.operationType = OperationType::Parting;
        m.operationName = "Parting";
      }
    ...
    try {
      ...
      case OperationType::Facing:
        settings.colorScheme =
            ToolpathDisplayObject::ColorScheme::OperationType;
      ...
      // Create a default tool for the display object (since
      // ToolpathDisplayObject needs it)
      auto defaultTool =
          std::make_shared<Tool>(Tool::Type::Turning, "Display Tool");

      // Create a new toolpath with the same operation type and movements
      auto newToolpath = std::make_shared<Toolpath>(
          "Display Toolpath", defaultTool, toolpath->getOperationType());
      ...
      Handle(ToolpathDisplayObject) displayObj =
          ToolpathDisplayObject::create(sharedToolpath, settings);

      if (!displayObj.IsNull()) {
        // Apply workpiece transformation so toolpaths align with the part
...
void ToolpathGenerationPipeline::reportProgress(double progress,
  ...
  std::cout << "ToolpathGenerationPipeline: "
            << static_cast<int>(progress * 100) << "% - " << status
            << std::endl;
}

// IMPROVED: Create Part object from stored geometry
std::unique_ptr<IntuiCAM::Geometry::OCCTPart>
ToolpathGenerationPipeline::createPartFromGeometry() const {
  if (!m_currentPartGeometry.IsNull()) {
    // Create Part object from actual stored geometry
    return std::make_unique<IntuiCAM::Geometry::OCCTPart>(
        &m_currentPartGeometry);
  } else {
