...
#include "../include/aistoolpathdisplay.h"
...
ToolpathManager::ToolpathManager(QObject *parent)
    : QObject(parent), m_workpieceManager(nullptr) {}

ToolpathManager::~ToolpathManager() { clearAllToolpaths(); }
...
void ToolpathManager::initialize(Handle(AIS_InteractiveContext) context) {
  ...
  if (m_context.IsNull()) {
    emit errorOccurred(
        "Failed to initialize ToolpathManager: AIS context is null");
  } else {
...
bool ToolpathManager::displayToolpath(
    const IntuiCAM::Toolpath::Toolpath &toolpath, const QString &name) {
  if (m_context.IsNull()) {
  ...
  // Copy toolpath data and create AIS_ToolpathDisplay
  std::shared_ptr<IntuiCAM::Toolpath::Toolpath> tp =
      std::make_shared<IntuiCAM::Toolpath::Toolpath>(toolpath);

  Handle(AIS_ToolpathDisplay) toolpathAIS =
      new AIS_ToolpathDisplay(tp, name.toStdString());

  // Apply current workpiece transformation
  gp_Trsf transformation = getWorkpieceTransformation();
  toolpathAIS->SetTransformation(transformation);

  // Display the toolpath
  m_context->Display(toolpathAIS, Standard_False);

  // Store AIS object and toolpath data
  m_displayedToolpaths[name] = toolpathAIS;
  m_toolpathData[name] = tp;
...
int ToolpathManager::displayToolpaths(
    const std::vector<std::shared_ptr<IntuiCAM::Toolpath::Toolpath>> &toolpaths,
...
void ToolpathManager::removeToolpath(const QString &name) {
  ...
  if (m_displayedToolpaths.contains(name)) {
    Handle(AIS_ToolpathDisplay) toolpathAIS = m_displayedToolpaths[name];
...
void ToolpathManager::setAllToolpathsVisible(bool visible) {
  ...
  for (auto it = m_displayedToolpaths.begin(); it != m_displayedToolpaths.end();
       ++it) {
    Handle(AIS_ToolpathDisplay) toolpathAIS = it.value();
    if (!toolpathAIS.IsNull()) {
      if (visible) {
...
bool ToolpathManager::areToolpathsVisible() const {
  ...
  for (auto it = m_displayedToolpaths.constBegin();
       it != m_displayedToolpaths.constEnd(); ++it) {
    Handle(AIS_ToolpathDisplay) ais = it.value();
...
void ToolpathManager::setDisplaySettings(
    const ToolpathDisplaySettings &settings) {
...
void ToolpathManager::updateAllToolpathVisualizations() {
  ...
  for (auto it = m_displayedToolpaths.begin(); it != m_displayedToolpaths.end();
    ...
    if (!toolpathAIS.IsNull()) {
      ...
      Quantity_Color rapid(m_displaySettings.rapidColor.redF(),
                           m_displaySettings.rapidColor.greenF(),
                           m_displaySettings.rapidColor.blueF(),
                           Quantity_TOC_RGB);
      Quantity_Color cut(m_displaySettings.cuttingColor.redF(),
                         m_displaySettings.cuttingColor.greenF(),
                         m_displaySettings.cuttingColor.blueF(),
                         Quantity_TOC_RGB);

      toolpathAIS->SetMoveColors(
          rapid, cut, Quantity_Color(0.0, 0.0, 1.0, Quantity_TOC_RGB));
      toolpathAIS->SetLineWidth(m_displaySettings.cuttingLineWidth);
...
TopoDS_Shape ToolpathManager::createToolpathShape(
    const IntuiCAM::Toolpath::Toolpath &toolpath) {
  const std::vector<IntuiCAM::Toolpath::Movement> &movements =
      toolpath.getMovements();
  ...
  for (size_t i = 1; i < movements.size(); ++i) {
    if (movements[i].type != currentType) {
      ...
      TopoDS_Shape segmentShape =
          createToolpathSegment(movements, segmentStart, i - 1);
  ...
  TopoDS_Shape finalSegment =
      createToolpathSegment(movements, segmentStart, movements.size() - 1);
...
TopoDS_Shape ToolpathManager::createToolpathSegment(
    const std::vector<IntuiCAM::Toolpath::Movement> &movements, size_t startIdx,
    size_t endIdx) {
  ...
  if (startIdx == endIdx) {
    ...
    // Create a small edge as a placeholder (could be replaced with a more
    // visible marker)
...
void ToolpathManager::setToolpathDisplayProperties(Handle(AIS_Shape) aisObject,
                                                   bool isRapid) {
  if (aisObject.IsNull()) {
    return;
  }
  ...
  Quantity_Color lineColor =
      isRapid ? Quantity_Color(m_displaySettings.rapidColor.redF(),
                               m_displaySettings.rapidColor.greenF(),
                               m_displaySettings.rapidColor.blueF(),
                               Quantity_TOC_RGB)
  ...
  // Set line width
  double lineWidth = isRapid ? m_displaySettings.rapidLineWidth
                             : m_displaySettings.cuttingLineWidth;
  ...
  Handle(Prs3d_LineAspect) lineAspect =
      new Prs3d_LineAspect(lineColor, lineType, lineWidth);
...
Quantity_Color ToolpathManager::getMovementColor(
    const IntuiCAM::Toolpath::Movement &movement) {
  ...
  case IntuiCAM::Toolpath::MovementType::ToolChange:
    return Quantity_Color(1.0, 0.5, 0.0,
                          Quantity_TOC_RGB); // Orange for tool change
...
void ToolpathManager::applyWorkpieceTransformationToToolpaths() {
  ...
  if (!m_workpieceManager) {
    qDebug() << "ToolpathManager: Cannot apply transformations - no workpiece "
                "manager set";
  ...
  qDebug() << "ToolpathManager: Applying transformations to"
           << m_displayedToolpaths.size() << "toolpaths";
  ...
  qDebug() << "  - Translation vector:" << translation.X() << ","
  ...
  for (const QString &name : keys) {
    ...
    if (!m_toolpathData.contains(name)) {
      qDebug() << "ToolpathManager: ERROR - No toolpath data found for" << name;
      failureCount++;
      continue;
    }

    std::shared_ptr<IntuiCAM::Toolpath::Toolpath> tp = m_toolpathData[name];
    Handle(AIS_ToolpathDisplay) newToolpathAIS =
        new AIS_ToolpathDisplay(tp, name.toStdString());
    newToolpathAIS->SetTransformation(transformation);
  ...
  m_context->UpdateCurrentViewer();
  qDebug() << "ToolpathManager: Transformation complete -" << successCount
...
gp_Trsf ToolpathManager::getWorkpieceTransformation() const {
  ...
  if (!m_workpieceManager) {
    qDebug() << "ToolpathManager: No workpiece manager set, returning identity "
                "transformation";
  ...
  qDebug() << "  - Workpiece position offset:"
           << m_workpieceManager->getWorkpiecePositionOffset() << "mm";
  qDebug() << "  - Workpiece flipped:"
           << (m_workpieceManager->isWorkpieceFlipped() ? "Yes" : "No");
  qDebug() << "  - Has axis alignment:"
           << (m_workpieceManager->hasAxisAlignmentTransformation() ? "Yes"
                                                                    : "No");
...
bool ToolpathManager::displayLatheProfile(
    const std::vector<IntuiCAM::Geometry::Point2D> &profile,
    const QString &name) {
  if (m_context.IsNull()) {
  ...
  try {
    ...
    if (transformation.Form() != gp_Identity) {
      BRepBuilderAPI_Transform transformer(compound, transformation,
                                           Standard_True);
    ...
    qDebug() << "Profile displayed:" << name << "with" << profile.size()
             << "points";
    return true;

  } catch (const Standard_Failure &e) {
    QString errorMsg = QString("OpenCASCADE error displaying profile: %1")
