 ...
 * - Different display styles for different movement types (rapid, cutting,
 * etc.)
...
public:
   ...
   */
  bool displayToolpath(const IntuiCAM::Toolpath::Toolpath &toolpath,
                       const QString &name);
   ...
   */
  int displayToolpaths(
      const std::vector<std::shared_ptr<IntuiCAM::Toolpath::Toolpath>>
          &toolpaths,
   ...
   */
  const ToolpathDisplaySettings &getDisplaySettings() const {
  ...
  bool
  displayLatheProfile(const std::vector<IntuiCAM::Geometry::Point2D> &profile,
...
private:
   ...
   */
  TopoDS_Shape
  createToolpathShape(const IntuiCAM::Toolpath::Toolpath &toolpath);
   ...
   */
  TopoDS_Shape createToolpathSegment(
...
private:
  ...
  QMap<QString, Handle(AIS_ToolpathDisplay)> m_displayedToolpaths;
  // Store toolpath data to keep them alive for redisplay
  QMap<QString, std::shared_ptr<IntuiCAM::Toolpath::Toolpath>> m_toolpathData;
